...
MetronicApp.controller('ServiceController', function ($rootScope, $scope, $http, $timeout, assignedservice) {
    ...
    //    assignedservice.submitPayment($scope.payment)
    //            .success(function (data) {
    ...
    $scope.saveAssignedServices = function (form) {
        ...
        if ($('#formAssignedServices').valid()) {
            var rStart = $scope.assignedService.StartDate.split('/');
            var dStart = rStart[1] + '/' + rStart[0] + '/' + rStart[2];
            var rEnd = $scope.assignedService.EndDate.split('/');
...
MetronicApp.factory('assignedservice', function ($http) {
    ...
    //assignedservice.submitPayment = function (model) {
    //    return $http({
    //        method: "POST",
    //        url: redirectionUrl + "Service/SubmitPayment",
    //        data: JSON.stringify(model),
    //    });
    //};
